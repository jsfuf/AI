...
const assistantStreamHandler = async (req: express.Request, res: express.Response) => {
  try {
    ...
    nvidiaApiKey = nvidiaApiKey || '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';
...
app.post(['/api/assistant/stream', '/assistant/stream'], assistantStreamHandler);
app.post(['/api/opencode/stream', '/opencode/stream'], assistantStreamHandler);

app.post(['/api/gemini/stream', '/gemini/stream'], async (req, res) => {
  try {
    const { messages, systemInstruction } = req.body;
    let apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey && process.env.API_KEY && process.env.API_KEY.startsWith('AIza')) {
...
app.post(['/api/gemini/memory', '/gemini/memory'], async (req, res) => {
  try {
